...
### :bear: Job3 : Train your model and predict accuracy or metrics.

### :dolphin: Job4 : if metrics accuracy is less than 80%  , then tweak the machine learning model architecture.
### :penguin: Job5 : Retrain the model or notify that the best model is being created

### :sun_with_face: Job6 : Create One extra job job6 for monitor : If container where app is running. fails due to any reason then this job should automatically start the container again from where the last trained model left

# :fire: EXPLANATION
## :trollface: JOB 1:-
#### :eyes: Created a job named job1 for retrive the data from github ,the developer when commits it pushes to github automatically using git post-commit hook and github webhook triggered job job1 is pull the data from the github and store in the task3 directory. in case directory alredy created then rm the directory and make new directory and store the github data.

### :mouse2: Pull the data from github repo.
![job](img/job1.jpg)

### :whale2: Copy github pull data in local task3 directory
![job](img/job1_1.jpg)

### :turtle: After running job1
![job](img/job1_3.jpg)
...
### :headphones: Trigger when job1 succesfully run
![job2](img/job2.jpg)

### :rainbow: For creating a container i use below script
![job2](img/job2_1.jpg)
...
## :pushpin: trigger when the job2,job3,job4 is faild
![job6](img/job6.jpg)

## :bug: Running the below script
![job6](img/job6_1.jpg)

## :sunglasses: Visual of our automation using build pipeline plugin
